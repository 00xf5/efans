    ...
    return (
        <div className="relative w-full h-full flex bg-zinc-950 overflow-hidden font-display">
            ...
            <div className="hidden lg:flex w-1/2 relative h-full overflow-hidden">
                ...
                {/* Subtle light leak */}
                <div className="absolute top-[-20%] right-[-20%] w-[80%] h-[80%] bg-pink-500/10 rounded-full blur-[160px] animate-pulse"></div>
            </div>
            ...
            <div className="w-full lg:w-1/2 h-full overflow-y-auto scrollbar-hide flex items-center lg:items-start justify-center p-8 md:p-20 lg:pt-32 bg-white lg:bg-white relative">
                ...
                <div className="w-full max-w-lg space-y-12 relative z-10">
                    ...
                    <div className="p-1.5 bg-zinc-100 dark:bg-zinc-900/50 rounded-[2.5rem] flex relative border border-zinc-200">
                        ...
                        <button
                            onClick={() => setRole('fan')}
                            className={`flex-1 py-4 rounded-[2.2rem] text-[11px] font-black uppercase tracking-widest relative z-10 transition-all ${role === 'fan' ? 'text-white' : 'text-zinc-400'}`}
                        >
                            I am a Fan
                        </button>
                        <div className={`absolute top-1.5 bottom-1.5 w-[calc(50%-6px)] bg-zinc-950 rounded-[2.2rem] shadow-2xl transition-all duration-500 ease-out-back ${role === 'fan' ? 'left-[calc(50%+3px)]' : 'left-1.5'}`}></div>
                    </div>
                    ...
                    <form className="space-y-8" onSubmit={(e) => e.preventDefault()}>
                        <div className="space-y-6">
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest block ml-6 leading-none">Security Key (Password)</label>
                                <input
                                    type="password"
                                    placeholder="••••••••••••"
                                    className="w-full bg-zinc-50 border border-zinc-100 px-8 py-6 rounded-[2rem] font-bold text-zinc-950 outline-none focus:border-pink-200 transition-all text-lg shadow-inner"
                        ...
                        <div className="space-y-6 pt-4">
                            ...
                            <p className="text-center text-[11px] text-zinc-400 font-bold italic translate-y-2">
                                Already sovereign? <Link to="/login" className="text-pink-500 hover:text-pink-600 transition-colors">Sign in to your Hub</Link>
                            </p>
                        </div>
                    </form>
