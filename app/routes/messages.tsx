    ...
    return (
        <div className="fixed inset-0 top-16 bg-[#FAFAFA] text-zinc-900 flex justify-center selection:bg-pink-100 overflow-hidden font-display">
            ...
            <div className="fixed inset-0 pointer-events-none opacity-40">
                ...
                <div className="absolute bottom-[-20%] right-[-10%] w-[70%] h-[70%] bg-violet-100/30 rounded-full blur-[160px] animate-pulse"></div>
            </div>
            ...
            <div className="w-full max-w-[1800px] h-full flex relative z-10 px-6 py-8 gap-8">
                    ...
                    <header className="flex flex-col gap-4">
                        <div className="flex justify-between items-end">
                            ...
                            <button
                                ...
                                className="text-[8px] font-black text-pink-500 uppercase tracking-widest border border-pink-100 px-3 py-1 rounded-full hover:bg-pink-50 transition-colors"
                            >
                                Switch Perspective
                            </button>
                        </div>
                        ...
                        {conversations.map((conv) => (
                            ...
                            >
                                <div className="relative">
                                    ...
                                    <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 ${activeId === conv.id ? 'border-zinc-900' : 'border-white'} ${conv.user.status === 'Online' ? 'bg-emerald-500' : 'bg-zinc-300'}`}></div>
                                </div>
                                <div className="flex-grow min-w-0">
                                    <div className="flex justify-between items-center mb-1">
                                        <h4 className="text-[11px] font-black uppercase tracking-widest truncate">{conv.user.name}</h4>
                                        <span className={`text-[8px] font-black uppercase tracking-widest ${activeId === conv.id ? 'text-zinc-500' : 'text-zinc-300'}`}>Active</span>
                                    </div>
                                    <p className={`text-[10px] font-bold italic truncate ${activeId === conv.id ? 'text-white/60' : 'text-zinc-400'}`}>
                ...
                <main className="flex-grow flex flex-col bg-white rounded-[4rem] border border-zinc-100 shadow-sm relative overflow-hidden animate-entrance [animation-delay:100ms] opacity-0 [animation-fill-mode:forwards]">
                    ...
                    <header className="px-10 py-8 border-b border-zinc-50 flex items-center justify-between bg-white/50 backdrop-blur-md z-20">
                        ...
                        <div className="flex items-center gap-3">
                            <button className="w-12 h-12 bg-zinc-50 rounded-2xl flex items-center justify-center text-zinc-400 hover:text-pink-500 hover:bg-pink-50 transition-all">
                ...
                <aside className="hidden xl:flex flex-col w-80 h-full animate-entrance [animation-delay:200ms] opacity-0 [animation-fill-mode:forwards]">
                    <div className="bg-white rounded-[4rem] border border-zinc-100 p-10 flex flex-col items-center text-center shadow-sm">
                        ...
                        <h4 className="text-2xl font-black italic text-zinc-900 tracking-tighter">{activeConv.user.name}</h4>
                        <p className="text-[10px] font-black text-pink-500 uppercase tracking-widest mt-1 mb-8">@{activeConv.user.name.toLowerCase().replace(' ', '')}</p>
                        ...
                        <div className="grid grid-cols-2 gap-4 w-full mb-10">
                            <div className="bg-zinc-50 p-4 rounded-3xl border border-zinc-100">
                                ...
                                <p className="text-[11px] font-bold text-zinc-900">Active Soul</p>
                            </div>
                            <div className="bg-zinc-50 p-4 rounded-3xl border border-zinc-100">
                                <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-1">Tier</p>
                                <p className="text-[11px] font-bold text-pink-500">Sovereign</p>
                            </div>
                        </div>

                        <div className="w-full space-y-4 pt-10 border-t border-zinc-50">
                            <h5 className="text-[9px] font-black text-zinc-300 uppercase tracking-[0.4em] italic mb-4">Quick Resonance</h5>
                            <button
                                onClick={handleGift}
                                className="w-full py-4 bg-zinc-900 text-white rounded-2xl text-[9px] font-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-xl"
                            >
                                Gift Glimpse
                            </button>
                            <button
                                onClick={() => showToast("Resonance Reported for Sanitization")}
                                className="w-full py-4 border border-zinc-100 text-zinc-400 rounded-2xl text-[9px] font-black uppercase tracking-widest hover:bg-zinc-50 transition-all"
                            >
                                Report Resonance
                            </button>
                        </div>
